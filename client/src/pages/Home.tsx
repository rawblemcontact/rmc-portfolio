...
const PHONE_W = 384;
const PHONE_H = 224;
// Expanded: cover viewport below header, front and center
const EXPANDED_WIDTH = "92vw";
const EXPANDED_MAX_W = 1280;
const EXPANDED_HEIGHT_VH = "72vh";

type SkillCardId = "core" | "tools";

const STARTUP_DUR = 0.12;
const FADE_DUR = 0.4;
const LINGER_MS = 500;
const EXPAND_DELAY_MS = STARTUP_DUR * 1000 + FADE_DUR * 1000 + LINGER_MS;
...
const SkillCardMorph = ({
  ...
  const isSlidingToCenter = activeCard === id && expandedCard === null;
  ...
  return (
    <motion.div
      ...
      transition={{
        duration: isOtherExpanded || isSlidingToCenter ? FADE_DUR : dur,
      ...
      style={{
        ...
        width: isExpanded ? EXPANDED_WIDTH : undefined,
        maxWidth: isExpanded ? EXPANDED_MAX_W : undefined,
    ...
    >
      <motion.div
        ...
        animate={{
          width: isExpanded ? "100%" : PHONE_W,
          height: isExpanded ? EXPANDED_HEIGHT_VH : PHONE_H,
          y: isExpanded ? -24 : 0,
        ...
        style={{ flex: isExpanded ? "1 1 100%" : "0 0 auto" }}
      >
        {isExpanded ? (
          <>
            <div
              className="absolute inset-0 rounded-[inherit] bg-black overflow-y-auto border-2 border-white/40"
              style={{ borderRadius: "inherit" }}
              aria-hidden
            />
            <motion.div
              className={`absolute inset-0 rounded-[inherit] ${accentClass} pointer-events-none`}
              style={{ borderRadius: "inherit" }}
              initial={reducedMotion ? { opacity: 0 } : { opacity: 0.88 }}
              animate={{ opacity: 0 }}
              transition={{
...
const SkillArsenal = () => {
  ...
  useEffect(() => {
    if (activeCard !== null) {
      const t = setTimeout(() => setExpandedCard(activeCard), reducedMotion ? 0 : EXPAND_DELAY_MS);
