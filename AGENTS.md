...
- **Palette yellow** is `#ffe73a` — same as the hero lockup yellow square. `--palette-yellow-projects` must stay `var(--palette-yellow)`. Hero squares: red `#ef3a2c`, yellow `#ffe73a` (menu-matched).
